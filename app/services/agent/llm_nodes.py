...
class GigaChatNodes:
    ...
    def think_node(self, state: AgentState) -> dict[str, Any]:
        ...
        messages = [
            SystemMessage(
                content=f"""
Ты эксперт по визуализации данных. 
Если пользователь просит ВИЗУАЛИЗИРОВАТЬ — идеально осознай данные csv_data и выбери лучший метод визуализации, ОБЯЗАТЕЛЬНО СЛЕДУЙ предпочтеням пользователя по дизайну.
Если пользователь просит СКОРРЕКТИРОВАТЬ РЕЗУЛЬТАТЫ — на основе used_methods вызови те же методы, но с изменёнными параметрами.
Если errors не пустой, то ИСПРАВЬ все свои ошибки.
